...
class LightDeviceActivity : AppCompatActivity() {
    private lateinit var binding:ActivityLightDeviceBinding;
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding = ActivityLightDeviceBinding.inflate(layoutInflater);
    ...
    private fun checkStateLight() {
        binding.loadingOverlay.visibility = View.VISIBLE
        val url = "http://$ipAddress/check";
                    ...
                    if(response.isSuccessful){
                        if(responseBody!= null){
                            ...
                            if(responseBody.equals("ON")){
                                ...
                                binding.imgLight.setImageResource(R.drawable.baseline_lightbulb_24);
                            }else{
                                binding.switch1.isChecked = false
                                binding.imgLight.setImageResource(R.drawable.baseline_lightbulb_outline_24)
                            }
                            binding.loadingOverlay.visibility = View.GONE
                        }
    ...
    private fun setBtnPower() {
        binding.switch1.setOnCheckedChangeListener{_, isChecked ->
            ...
            if(isChecked){
                ...
                binding.txtOn.visibility = View.VISIBLE;
                binding.imgLight.setImageResource(R.drawable.baseline_lightbulb_24);
                senCmdToDev("led/on");
            }else{
                binding.txtOFF.visibility = View.VISIBLE;
                binding.txtOn.visibility = View.GONE;
                binding.imgLight.setImageResource(R.drawable.baseline_lightbulb_outline_24)
                senCmdToDev("led/off");
    ...
    private fun senCmdToDev(cmd:String){
        ...
        client.newCall(request).enqueue(object : Callback{
            override fun onFailure(call: Call, e: IOException) {
                runOnUiThread{
                    Toast.makeText(this@LightDeviceActivity,"Lỗi kết nối",Toast.LENGTH_SHORT).show()
                    binding.switch1.isEnabled = true;
                }
            }
            ...
            override fun onResponse(call: Call, response: Response) {
                runOnUiThread{
                    if(response.isSuccessful){
                        Toast.makeText(this@LightDeviceActivity,"Ok",Toast.LENGTH_SHORT).show()
                    }else{
                        Toast.makeText(this@LightDeviceActivity,"Lỗi yêu cầu",Toast.LENGTH_SHORT).show()
                    }
                    binding.switch1.isEnabled = true;
                }
                response.close();
